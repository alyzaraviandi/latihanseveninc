<x-layout>
    <x-slot:title>
        Professors
    </x-slot:title>

    <div class="mb-4 flex space-x-4">
        <x-button href="{{ route('professors.create') }}" class="bg-blue-600 hover:bg-blue-700">
            Create Professors
        </x-button>
    </div>


    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    <th scope="col" class="px-6 py-3">No</th>
                    <th scope="col" class="px-6 py-3">Professor Number</th>
                    <th scope="col" class="px-6 py-3">Name</th>
                    <th scope="col" class="px-6 py-3">NIP</th>
                    <th scope="col" class="px-6 py-3">Classes</th>
                    <th scope="col" class="px-6 py-3">Action</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($professors as $professor)
                    <tr
                        class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <td class="px-6 py-4">{{ $professor->id }}</td>
                        <td class="px-6 py-4">{{ $professor->prof_number }}</td>
                        <td class="px-6 py-4">{{ $professor->name }}</td>
                        <td class="px-6 py-4">{{ $professor->nip }}</td>
                        <td class="px-6 py-4">
                            @if ($professor->classes->isEmpty())
                                <p>No classes assigned</p>
                            @else
                                <ul>
                                    @foreach ($professor->classes as $class)
                                        <li>{{ $class->name }}</li>
                                    @endforeach
                                </ul>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                            <a href="{{ route('professors.show', $professor->id) }}"
                                class="font-medium text-blue-600 dark:text-blue-500 hover:underline">View</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
        </table>

        <nav class="flex items-center flex-column flex-wrap md:flex-row justify-between pt-4 bg-white shadow-md rounded-lg p-4 border border-gray-200"
            aria-label="Table navigation">
            <span
                class="text-sm font-normal text-gray-500 dark:text-gray-400 mb-4 md:mb-0 block w-full md:inline md:w-auto mr-4">
                Showing {{ $professors->firstItem() }} to {{ $professors->lastItem() }} of {{ $professors->total() }}
                results
            </span>
            <ul class="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
                <!-- Pagination links -->
                @if ($professors->hasPages())
                    <li>
                        <a href="{{ $professors->previousPageUrl() }}"
                            class="px-3 py-2 border border-gray-300 rounded-lg text-gray-500 bg-white hover:bg-gray-100">Previous</a>
                    </li>
                    @for ($i = 1; $i <= $professors->lastPage(); $i++)
                        <li>
                            <a href="{{ $professors->url($i) }}"
                                class="px-3 py-2 border border-gray-300 rounded-lg {{ $i == $professors->currentPage() ? 'bg-blue-500 text-white' : 'bg-white text-gray-500 hover:bg-gray-100' }}">
                                {{ $i }}
                            </a>
                        </li>
                    @endfor
                    <li>
                        <a href="{{ $professors->nextPageUrl() }}"
                            class="px-3 py-2 border border-gray-300 rounded-lg text-gray-500 bg-white hover:bg-gray-100">Next</a>
                    </li>
                @endif
            </ul>
        </nav>
    </div>



</x-layout>
